...
This page is for administrators operating chat. The Chat core service and relevant modules must be enabled; global/private sends, mutes, and colored profiles require administrator permission. Chat can contain personal information, so apply the configured retention policy.
...
- Global messages and no-prefix commands increase the chance of accidental execution. Use an explicit prefix and a test account before production changes.
- Chat history retention `0` means keep indefinitely, increasing sensitive-data retention. Check policy and database capacity before changing it.
