...
This page is for administrators with player-query or management permissions. Player List requires a reachable backend; Kick, Ban, Mute, item grants, profile reset, and teleport also require the relevant permission and can create audit records. Player Profile is not a sidebar entry: it is a hidden detail route opened from Player List, chat, game events, or another player link.
