...
> For administrators investigating player locations, claims, entities, or tracks. Requires Map Info, usable map rendering/tiles, and the relevant modules; dragging an online player to teleport, claim operations, and vehicle-inventory reads also require higher permission and a readable loaded area.
...
- Confirm Map Info, map rendering/tiles, Player Tracking, and any required entity modules are available.
...
::: warning
Map data is affected by backend sampling, rendering, clustering, loaded areas, and refresh intervals. Offline players and historical tracks are not live positions. Hide player IDs, IPs, server coordinates, and file paths in shared screenshots.
:::
