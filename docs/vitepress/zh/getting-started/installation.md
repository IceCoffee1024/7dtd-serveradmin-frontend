...
> 面向负责部署的管理员。需要可维护的 7DTD 服务器、`<7DTD_SERVER_ROOT>`、后端源码或发布物、Node.js、pnpm 和浏览器可访问的 `<SERVERADMIN_API_BASE_URL>`。需要能在维护窗口重启游戏服务器并读取 Swagger。
...
- 准备服务器根目录、后端发布物、前端工作区和维护窗口。
- 确认记录中的 API 地址已经脱敏，并且不会把密码或令牌放入 `VITE_*` 变量。
- 确认反向代理或静态站点能够路由 `/api`、`/swagger` 和前端页面路径。
