...
本页面向管理聊天的管理员。Chat 核心服务和对应模块必须启用；发送全局/私聊、禁言以及彩色配置需要管理员权限。聊天内容可能包含玩家个人信息，处理时遵守服务器保留策略。
...
- 全局消息和无前缀命令会扩大误触范围；先在测试频道/账号验证，生产环境使用明确前缀。
- Chat history 的 `0` 表示长期保留，会增加敏感数据留存；变更保留天数前确认合规和数据库容量。
