...
> 面向有玩家查询或管理权限的管理员。需要后端在线；Kick、Ban、Mute、发放物品、重置资料和传送等操作还需要对应权限，并可能触发审计。Player Profile 是从玩家列表、聊天、游戏事件或其他玩家链接进入的隐藏详情路由。
...
- 确认 Player List 以及追踪、库存和领地容器数据所需模块已启用。
- 准备用于身份核对的稳定 Player ID 或平台 ID；只凭显示名或 Entity ID 不足以确认身份。
- 在执行有影响的操作前准备维护窗口和回滚或审计方案。
