...
> 面向调查玩家位置、领地、实体或追踪轨迹的管理员。需要 Map Info、可用的地图渲染/图块和相应模块；在线玩家拖拽传送、领地操作或车辆库存读取还需要更高权限和已加载的游戏区域。
...
- 确认 Map Info、地图渲染/图块、Player Tracking 和所需实体模块可用。
...
::: warning
地图数据受后端采样、地图渲染、聚合、加载区域和刷新间隔影响；离线玩家和历史轨迹不等于实时位置。共享截图前隐藏玩家 ID、IP、服务器坐标和文件路径。
:::
