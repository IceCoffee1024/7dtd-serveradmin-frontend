...
> 面向有控制台、审计和游戏事件读取权限的管理员。需要命令权限、已启用模块和可达的游戏进程；控制台只展示后端允许的命令。
...
- 确认账号能读取所需审计和游戏事件记录，并且只允许使用预期的控制台命令。
- 准备一个只读测试命令、时间范围和稳定的玩家或资源标识用于日志检索。
- 对改变状态的命令，打开控制台前确认维护窗口、备份、目标和回滚方案。
...
::: warning
允许列表和 Permission 决定命令可见/可执行范围；`inMainThread: true` 不等于异步，日志也受模块启用、保留策略和数据库可用性影响。
:::

::: danger
控制台可改变世界、玩家、配置和进程。先做备份、确认目标、维护窗口和回滚方案；危险命令只在明确授权后执行。
:::
