...
defineOptions({ name: 'TeleportPage' });
...
async function submitToPosition() {
  ...
  const confirmed = await confirm({
    text: t('views.teleport.messages.confirmToPos', { name, x, y, z }),
    type: 'warning',
  });
  if (!confirmed)
    return;
  ...
  try {
    ...
    toast({ type: 'success', title: t('views.teleport.messages.success') });
  }
  catch (error) {
    console.error(error);
  }
...
async function submitToPlayer() {
  ...
  const confirmed = await confirm({
    text: t('views.teleport.messages.confirmToPlayer', { source, target }),
    type: 'warning',
  });
  if (!confirmed)
    return;
...
<template>
  <div class="flex flex-col gap-4">
    <el-tabs type="border-card">
      ...
      <el-tab-pane :label="t('views.teleport.tabs.toPosition')">
        <div class="p-4 max-w-lg">
          <div class="mb-4 flex gap-2 items-center">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ t('views.teleport.hints.onlinePlayers') }}</span>
          ...
          <el-form label-position="top" @submit.prevent>
            <el-form-item :label="t('views.teleport.fields.sourcePlayer')">
              <el-select
                v-model="toPositionForm.entityId"
                :placeholder="t('views.teleport.placeholders.selectPlayer')"
                filterable
                clearable
                class="w-full"
              >
            ...
            <el-form-item>
              ...
              >
                {{ t('views.teleport.actions.teleport') }}
              </el-button>
            </el-form-item>
          </el-form>
        </div>
      ...
      <el-tab-pane :label="t('views.teleport.tabs.toPlayer')">
        <div class="p-4 max-w-lg">
          <div class="mb-4 flex gap-2 items-center">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ t('views.teleport.hints.onlinePlayers') }}</span>
            <IconButton button-size="small" icon-size="16" :loading="loadingPlayers" :tooltip-content="t('components.myTable.refresh')" @click="refreshPlayers">
          ...
          <el-form label-position="top" @submit.prevent>
            ...
            <el-form-item :label="t('views.teleport.fields.targetPlayer')">
              <el-select
