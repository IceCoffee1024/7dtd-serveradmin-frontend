...
/**
 * Registers hostiles layer.
 * @param context - Shared OpenLayers module context.
 */
export function setupHostilesLayer(context: OpenLayersModuleContext) {
  setupEntityLocationLayer(context, {
    layerId: LAYER_ID.HOSTILES_CLUSTER_LAYER,
    layerTitle: i18n.global.t('views.map.hostiles'),
    entityType: 'Hostiles',
    iconStyle: new Style({
      image: new Icon({
        src: new URL('~/assets/images/marker-zombie-2x.png', import.meta.url).href,
        anchor: [0.5, 0.5],
        scale: 0.6,
        opacity: 0.9,
    ...
    clusterFillColor: 'rgba(239, 68, 68, 0.85)',
    zIndex: 40,
  });
  ...
  //   image: new Icon({
  //     src: new URL('~/assets/images/marker-zombie.png', import.meta.url).href,
  //     anchor: [0.5, 0.5],
  //     scale: 1,
  //     crossOrigin: 'anonymous',
  //   }),
  ...
  //   zIndex: 31,
  // });
  // layer.set('id', HOSTILES_LAYER_ID);
  // layer.set('title', i18n.global.t('views.map.zombie'));
  ...
  //   setHostilesCount(nextFeatures.length);
  // };
  ...
  //       const hostileFeature = resolveSingleFeature(featureLike);
  //       if (!hostileFeature) {
  //         return undefined;
  //       }

  //       const coordinate = (hostileFeature.getGeometry() as Point).getCoordinates();
  //       const entityName = String(hostileFeature.get('entityName') ?? '-');
  //       const entityId = String(hostileFeature.get('entityId') ?? '-');

  //       popupElement.textContent = `${i18n.global.t('views.map.zombie')}: ${entityName} (${entityId})`;
