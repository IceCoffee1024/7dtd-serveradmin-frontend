...
import type { BackupFeatureSettingsDto, ServerConfigBackupSettingsDto } from '~/generated/api/types.gen';
...
import {
  ...
  backupRunServerConfigBackupMutation,
...
const policyFields = computed<MyFormField<FormModel>[]>(() => [
  {
    prop: 'isEnabled',
    label: t('views.backup.config.fields.isEnabled'),
    el: 'el-switch',
    props: {
      inlinePrompt: true,
...
const runBackupMutation = useMutation(backupRunServerConfigBackupMutation());
...
function applyValues(source?: ServerConfigBackupSettingsDto) {
...
watch(
  ...
  async (data) => {
    ...
    settings.value = data;
    applyValues(data.serverConfigBackup);
    await nextTick();
...
async function refreshSettings() {
  ...
  applyValues(state.data.serverConfigBackup);
  await nextTick();
  formRef.value?.clearValidate();
}
